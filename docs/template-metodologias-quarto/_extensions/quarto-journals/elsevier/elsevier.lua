...
return {
  {
    Meta = function(meta)
      ...
      if quarto.doc.is_format("pdf") then
        ...
        -- only emit the "Corresponding author" footnote definition when at
        -- least one author is actually marked as corresponding; otherwise
        -- elsarticle.cls prints the footnote text unconditionally even
        -- though no author reference points to it
        local hasCorrespondingAuthor = false
        if meta['by-author'] then
          for i, v in ipairs(meta['by-author']) do
            if v.attributes and v.attributes.corresponding then
              hasCorrespondingAuthor = true
            end
          end
        end
        meta['has-corresponding-author'] = hasCorrespondingAuthor
